...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `ファイル` | モデルのコンテキストとして含める入力ファイル。現時点ではテキスト（.txt）ファイルとPDF（.pdf）ファイルのみを受け付けます。ファイルは32MB未満である必要があります。 | COMBO | はい | 複数のオプションが利用可能（入力ディレクトリ内の32MB未満のすべての .txt および .pdf ファイル） |
| `OPENAI_INPUT_FILES` | このノードから読み込まれたファイルと一緒にバッチ処理する、オプションの追加ファイル。入力ファイルをチェーン接続できるため、1 つのメッセージに複数の入力ファイルを含めることができます。 | OPENAI_INPUT_FILES | いいえ | N/A |
...
| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `OPENAI_INPUT_FILES` | OpenAI API 呼び出しのコンテキストとして使用できるようフォーマットされた入力ファイル。 | OPENAI_INPUT_FILES |
