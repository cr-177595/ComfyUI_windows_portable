# 创建约束模型为LoRA

此节点通过加载检查点权重并对模型和 CLIP 组件应用强度调整，以 LoRA（低秩适应）方式创建钩子模型。它允许您通过基于钩子的方法对现有模型应用 LoRA 风格的修改，从而实现微调和适应，而无需永久更改模型。该节点可以结合之前的钩子，并缓存已加载的权重以提高效率。

## 输入

| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `Checkpoint名称` | 从中加载权重的检查点文件（从可用检查点中选择） | STRING | 是 | 多个选项可用 |
| `模型强度` | 应用于模型权重的强度乘数（默认值：1.0） | FLOAT | 是 | -20.0 到 20.0 |
| `CLIP强度` | 应用于 CLIP 权重的强度乘数（默认值：1.0） | FLOAT | 是 | -20.0 到 20.0 |
| `前一个约束` | 可选的先前钩子，用于与新创建的 LoRA 钩子结合 | HOOKS | 否 | - |

**参数约束：**

- `ckpt_name` 参数从可用的检查点文件夹中加载检查点
- 两个强度参数接受 -20.0 到 20.0 之间的值，步长为 0.01
- 当未提供 `prev_hooks` 时，节点会创建一个新的钩子组
- 节点会缓存已加载的权重，以避免重复加载同一检查点

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `HOOKS` | 创建的 LoRA 钩子，如果提供了先前钩子，则与其结合 | HOOKS |

> 本文档由 AI 生成。如果您发现任何错误或有改进建议，欢迎贡献！ [在 GitHub 上编辑](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/CreateHookModelAsLora/zh.md)

---
**Source fingerprint (SHA-256):** `8c0dd6b2e8e99e1d7dbc864aa802c0713842fb0d4ee018ea5cbedfb7896a770d`
