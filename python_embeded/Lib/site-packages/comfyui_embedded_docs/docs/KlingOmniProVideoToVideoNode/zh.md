...
| `提示词` | 描述视频内容的文本提示。可包含正面和负面描述。 | STRING | 是 | 不适用 |
| `宽高比` | 生成视频所需的宽高比。 | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` |
| `时长` | 生成视频的时长（秒），默认值为 3。 | INT | 是 | 3 到 10 |
| `参考视频` | 用作参考的视频。 | VIDEO | 是 | 不适用 |
| `保留原音频` | 决定输出中是否保留参考视频的音频（默认值：True）。 | BOOLEAN | 是 | 不适用 |
| `参考图像` | 最多 4 张额外的参考图像。 | IMAGE | 否 | 不适用 |
| `分辨率` | 生成视频的分辨率（默认值："1080p"）。 | COMBO | 否 | `"1080p"`<br>`"720p"` |
