...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `提示词` | 正向文本提示词 | STRING | 是 | - |
| `负面提示词` | 反向文本提示词 | STRING | 是 | - |
| `CFG` | 控制输出与提示词的贴合程度（默认值：0.75） | FLOAT | 否 | 0.0-1.0 |
| `宽高比` | 生成视频的宽高比（默认值："16:9"） | COMBO | 否 | "16:9"<br>"9:16"<br>"1:1"<br>"21:9"<br>"3:4"<br>"4:3" |
| `镜头控制` | 可使用 Kling 摄像机控制节点创建。控制视频生成过程中的摄像机移动和运动。 | CAMERA_CONTROL | 否 | - |

### 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `output` | 带有摄像机控制效果的生成视频 | VIDEO |
