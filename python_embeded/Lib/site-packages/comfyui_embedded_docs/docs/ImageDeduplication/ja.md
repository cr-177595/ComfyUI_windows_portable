...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `画像` | 重複除去処理を行う画像のバッチです。 | IMAGE | はい | - |
| `類似度しきい値` | 類似度の閾値（0～1）です。値が大きいほど類似度が高いことを示します。この閾値を超える画像は重複とみなされます。（デフォルト：0.95） | FLOAT | いいえ | 0.0 - 1.0 |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `画像` | 重複が除去されたフィルタリング済み画像のリストです。 | IMAGE |
