...
| --- | --- | --- | --- | --- |
| `视频` | 将从中移除背景的输入视频文件。 | VIDEO | 是 | 不适用 |
| `背景颜色` | 用作输出视频新背景的纯色。 | STRING | 是 | `"Black"`<br>`"White"`<br>`"Gray"`<br>`"Red"`<br>`"Green"`<br>`"Blue"`<br>`"Yellow"`<br>`"Cyan"`<br>`"Magenta"`<br>`"Orange"` |
...
| 输出名称 | 说明 | 数据类型 |
| --- | --- | --- |
| `output` | 已移除背景并替换为所选颜色的处理后视频文件。 | VIDEO |
