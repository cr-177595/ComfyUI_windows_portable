...
| `步数` | 生成 sigma 值所需的采样步数（默认值：20） | INT | 是 | 1 到 10000 |
| `alpha` | beta 调度器的 alpha 参数，控制调度曲线（默认值：0.6） | FLOAT | 是 | 0.0 到 50.0 |
| `beta` | beta 调度器的 beta 参数，控制调度曲线（默认值：0.6） | FLOAT | 是 | 0.0 到 50.0 |

## 输出

| 输出名称 | 说明 | 数据类型 |
| --- | --- | --- |
| `SIGMAS` | 用于采样过程的噪声水平序列（sigma 值） | SIGMAS |
