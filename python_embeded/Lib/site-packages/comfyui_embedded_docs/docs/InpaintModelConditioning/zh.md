...
| 参数 | 描述 | Comfy 数据类型 |
| --- | --- | --- |
| `正面条件` | 表示应用于修复模型的正面条件信息或参数。此输入对于定义修复操作的上下文或约束至关重要，会显著影响最终输出。 | `CONDITIONING` |
| `负面条件` | 表示应用于修复模型的负面条件信息或参数。此输入对于指定修复过程中应避免的条件或上下文至关重要，从而影响最终输出。 | `CONDITIONING` |
| `vae` | 指定条件处理过程中使用的 VAE 模型。此输入对于确定将使用的 VAE 模型的具体架构和参数至关重要。 | `VAE` |
| `像素` | 表示待修复图像的像素数据。此输入对于提供修复任务所需的视觉上下文至关重要。 | `IMAGE` |
| `遮罩` | 指定应用于图像的遮罩，指示需要修复的区域。此输入对于定义图像中需要修复的具体区域至关重要。 | `MASK` |
