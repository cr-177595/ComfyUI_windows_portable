...
| `on_false` | 当 `switch` 条件为假时输出的值。此输入为可选，但 `on_false` 或 `on_true` 中至少有一个必须连接。 | MATCH_TYPE | 否 |  |
| `on_true` | 当 `switch` 条件为真时输出的值。此输入为可选，但 `on_false` 或 `on_true` 中至少有一个必须连接。 | MATCH_TYPE | 否 |  |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `output` | 选定的值。它将与连接的 `on_false` 或 `on_true` 输入的数据类型匹配。 | MATCH_TYPE |
