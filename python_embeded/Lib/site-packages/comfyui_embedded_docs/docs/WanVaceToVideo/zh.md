...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `正面条件` | 用于引导生成的正面条件输入 | CONDITIONING | 是 | - |
| `负面条件` | 用于引导生成的负面条件输入 | CONDITIONING | 是 | - |
...
| `长度` | 视频帧数（默认值：81，步长：4） | INT | 是 | 1 至 MAX_RESOLUTION |
| `批量大小` | 同时生成的视频数量（默认值：1） | INT | 是 | 1 至 4096 |
| `强度` | 视频条件的控制强度（默认值：1.0，步长：0.01） | FLOAT | 是 | 0.0 至 1000.0 |
| `控制视频` | 用于控制条件的可选输入视频 | IMAGE | 否 | - |
| `控制遮罩` | 用于控制视频中哪些部分需要修改的可选遮罩 | MASK | 否 | - |
| `参考图像` | 用于额外条件的可选参考图像 | IMAGE | 否 | - |
