...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `image` | 参照として使用する画像。 | IMAGE | はい | - |
| `weight` | 画像参照の重み（デフォルト：1.0）。 | FLOAT | はい | 0.0 - 1.0 |
| `luma_ref` | 追加先となる既存のLuma参照チェーン（オプション）。 | LUMA_REF | いいえ | - |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `luma_ref` | 画像と重みを含むLuma参照チェーン。 | LUMA_REF |
