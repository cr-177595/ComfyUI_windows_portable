...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `模型任务ID` | 需要应用纹理的模型任务 ID | MODEL_TASK_ID | 是 | - |
...
| `纹理对齐` | 纹理对齐方法（默认值："original_image"）。"original_image" 将纹理对齐到原始输入图像，而 "geometry" 将其对齐到 3D 几何体。 | COMBO | 否 | "original_image"<br>"geometry" |
