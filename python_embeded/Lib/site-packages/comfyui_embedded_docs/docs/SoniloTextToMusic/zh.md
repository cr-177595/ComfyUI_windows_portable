...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `prompt` | 描述要生成音乐的文本提示词。此为必填字段。 | STRING | 是 | 不适用 |
| `duration` | 目标时长（秒）。设置为 0 可让模型根据提示词推断时长。最长：6 分钟（360 秒）。默认值：0。 | INT | 否 | 0 到 360 |
| `seed` | 用于可复现性的随机种子。目前 Sonilo 服务会忽略此参数，但保留以保持工作流一致性。默认值：0。 | INT | 否 | 0 到 18446744073709551615 |
