...
| Uniform | Description | Type |
| --- | --- | --- |
| `u_resolution` | **Output** framebuffer dimensions in pixels (`width, height`). This is the size you're writing to, which may differ from any input image's size when `尺寸模式` is `"custom"`. | `vec2` |
