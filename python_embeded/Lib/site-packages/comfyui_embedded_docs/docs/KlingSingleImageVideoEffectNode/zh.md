...
| --- | --- | --- | --- | --- |
| `图像3` | 参考图像。URL 或 Base64 编码字符串（不含 data:image 前缀）。文件大小不超过 10MB，分辨率不低于 300x300px，宽高比在 1:2.5 至 2.5:1 之间 | IMAGE | 是 | - |
| `效果` | 应用于视频生成的特效场景类型。部分特效可能采用不同的定价。 | COMBO | 是 | `"dizzydizzy"`<br>`"bloombloom"`<br>`"neon"`<br>`"cartoon"`<br>`"sketch"`<br>`"oil"`<br>`"watercolor"`<br>`"3d"` |
| `模型` | 用于生成视频特效的特定模型版本。 | COMBO | 是 | `"kling-v1-5"`<br>`"kling-v1-6"` |
