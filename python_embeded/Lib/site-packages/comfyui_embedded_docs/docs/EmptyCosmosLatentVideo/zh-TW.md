...
| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
| --- | --- | --- | --- | --- |
| `寬度` | 潛在影片的寬度（單位：像素，預設值：1280，必須能被 16 整除） | INT | 是 | 16 至 MAX_RESOLUTION |
| `高度` | 潛在影片的高度（單位：像素，預設值：704，必須能被 16 整除） | INT | 是 | 16 至 MAX_RESOLUTION |
| `長度` | 潛在影片的幀數（預設值：121，必須能被 8 整除） | INT | 是 | 1 至 MAX_RESOLUTION |
| `批次大小` | 單一批次中生成的潛在影片數量（預設值：1） | INT | 否 | 1 至 4096 |

## 輸出

| 輸出名稱 | 說明 | 資料類型 |
| --- | --- | --- |
| `samples` | 生成的空潛在影片張量，其值皆為零 | LATENT |
