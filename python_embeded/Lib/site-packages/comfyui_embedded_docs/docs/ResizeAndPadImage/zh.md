...
| --- | --- | --- | --- | --- |
| `图像` | 待调整大小并填充的输入图像 | IMAGE | 是 | - |
| `目标宽度` | 输出图像的期望宽度（默认值：512） | INT | 是 | 1 至 MAX_RESOLUTION |
| `目标高度` | 输出图像的期望高度（默认值：512） | INT | 是 | 1 至 MAX_RESOLUTION |
| `填充颜色` | 用于填充缩放后图像周围区域的颜色（默认值："white"） | COMBO | 是 | "white"<br>"black" |
| `插值方法` | 用于调整图像大小的插值方法（默认值："area"） | COMBO | 是 | "area"<br>"bicubic"<br>"nearest-exact"<br>"bilinear"<br>"lanczos" |

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `图像` | 调整大小并填充后的输出图像 | IMAGE |
