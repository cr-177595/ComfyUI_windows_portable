...
| `条件` | 需要添加 ControlNet 参数以增强的现有条件数据 | CONDITIONING | 是 | - |
| `文本` | 由 CLIP 模型处理的文本输入。支持多行文本和动态提示 | STRING | 是 | - |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `CONDITIONING` | 增强后的条件数据，包含从 CLIP 文本编码中提取的 ControlNet 交叉注意力参数（`cross_attn_controlnet` 和 `pooled_output_controlnet`） | CONDITIONING |
