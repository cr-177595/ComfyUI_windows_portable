...
| `reuse_threshold` | キャッシュされたステップを再利用するための閾値（デフォルト: 0.2）。 | FLOAT | いいえ | 0.0 - 3.0 |
| `start_percent` | EasyCache の使用を開始する相対的なサンプリングステップ（デフォルト: 0.15）。 | FLOAT | いいえ | 0.0 - 1.0 |
| `end_percent` | EasyCache の使用を終了する相対的なサンプリングステップ（デフォルト: 0.95）。 | FLOAT | いいえ | 0.0 - 1.0 |
