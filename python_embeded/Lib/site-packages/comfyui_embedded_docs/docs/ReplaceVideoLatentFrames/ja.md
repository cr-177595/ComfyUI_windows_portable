...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `destination` | フレームが置き換えられる宛先の潜在情報です。 | LATENT | はい | - |
| `source` | 宛先の潜在情報に挿入するフレームを提供するソースの潜在情報です。提供されない場合、宛先の潜在情報はそのまま返されます。 | LATENT | いいえ | - |
| `index` | 宛先の潜在情報内で、ソースの潜在フレームが配置される開始フレームインデックスです。負の値は末尾からのカウントを示します（デフォルト：0）。 | INT | はい | -MAX_RESOLUTION ～ MAX_RESOLUTION |
...
| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `output` | フレーム置き換え処理後の結果となる潜在ビデオです。 | LATENT |
