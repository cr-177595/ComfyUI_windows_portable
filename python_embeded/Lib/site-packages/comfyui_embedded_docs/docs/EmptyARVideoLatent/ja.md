...
| `batch_size` | 1回のバッチで生成する動画の数（デフォルト：1） | INT | はい | 1 ～ 64 |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `LATENT` | ゼロで満たされた潜在テンソル。指定された寸法、長さ、バッチサイズを持つ空の動画潜在空間を表します。テンソルの形状は [batch_size, 16, lat_t, height/8, width/8] で、lat_t は length から計算されます。 | LATENT |
