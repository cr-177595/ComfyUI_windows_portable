...
| 출력 이름 | 설명 | 데이터 타입 |
|-------------|-------------|-----------|
| `바운딩 박스` | 모든 경계 상자가 렌더링된 캔버스를 보여주는 RGB 이미지로, 레이블, 색상 팔레트 견본, 설명 텍스트가 포함됩니다. | IMAGE |
| `요소` | 픽셀 좌표로 표현된 경계 상자 목록으로, 각 상자는 x, y, 너비, 높이 값을 포함합니다. | BOUNDING_BOX |
| `elements` | 요소 객체의 구조화된 배열로, 각 객체는 유형, 경계 상자 좌표(0-1000으로 정규화), 텍스트(텍스트 유형의 경우), 설명, 색상 팔레트를 포함합니다. | ARRAY |
