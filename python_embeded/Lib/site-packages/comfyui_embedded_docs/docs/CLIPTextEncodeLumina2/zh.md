...
| --- | --- | --- | --- | --- |
| `系统提示词` | Lumina2 提供两种系统提示词："superior" 生成具有卓越图文对齐效果的图像；"alignment" 生成具有最高图文对齐程度的高质量图像。 | STRING | 是 | `"superior"`<br>`"alignment"` |
| `提示词` | 需要编码的文本。支持多行输入和动态提示词。 | STRING | 是 | 无 |
| `clip` | 用于编码文本的 CLIP 模型。 | CLIP | 是 | 无 |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `CONDITIONING` | 包含嵌入文本的条件数据，用于引导扩散模型。 | CONDITIONING |
