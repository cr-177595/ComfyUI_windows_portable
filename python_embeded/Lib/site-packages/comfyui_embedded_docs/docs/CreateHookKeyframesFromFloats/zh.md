...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `浮点强度` | 表示关键帧强度值的单个浮点值或浮点值列表（默认值：-1） | FLOATS | 是 | -1 到 ∞ |
| `开始百分比` | 时间轴上第一个关键帧的起始百分比位置（默认值：0.0） | FLOAT | 是 | 0.0 到 1.0 |
| `结束百分比` | 时间轴上最后一个关键帧的结束百分比位置（默认值：1.0） | FLOAT | 是 | 0.0 到 1.0 |
| `打印关键帧` | 启用时，将生成的关键帧信息打印到控制台（默认值：False） | BOOLEAN | 是 | True/False |
| `前一个约束关键帧` | 要将新关键帧添加到的现有钩子关键帧组，如果未提供则创建新组 | HOOK_KEYFRAMES | 否 | - |
