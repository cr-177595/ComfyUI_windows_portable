...
| --- | --- | --- | --- | --- |
| `LoRA名称` | 从 loras 目录加载的 LoRA 文件名 | STRING | 是 | 多个可用选项 |
| `模型强度` | 模型调整的强度乘数（默认值：1.0） | FLOAT | 是 | -20.0 到 20.0 |
| `CLIP强度` | CLIP 调整的强度乘数（默认值：1.0） | FLOAT | 是 | -20.0 到 20.0 |
| `前一个约束` | 可选的现有钩子组，用于与新的 LoRA 钩子合并 | HOOKS | 否 | 不适用 |

**参数约束：**

- 如果 `strength_model` 和 `strength_clip` 均设置为 0，节点将跳过创建新的 LoRA 钩子，直接返回未更改的现有钩子
- 节点会缓存最后加载的 LoRA 文件，以便在重复使用同一 LoRA 时优化性能

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `HOOKS` | 包含合并后的 LoRA 钩子及任何先前钩子的钩子组 | HOOKS |
