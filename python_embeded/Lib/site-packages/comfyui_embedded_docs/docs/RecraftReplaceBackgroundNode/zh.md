# Recraft 更换背景

根据提供的提示词替换图像背景。此节点使用 Recraft API 根据您的文本描述为图像生成新背景，让您能够在保持主体不变的同时彻底改变背景。

## 输入

| 参数 | 描述 | 数据类型 | 是否必填 | 取值范围 |
| --- | --- | --- | --- | --- |
| `图像` | 待处理的输入图像 | IMAGE | 是 | - |
| `提示词` | 图像生成的提示词（默认：空） | STRING | 是 | - |
| `数量` | 生成的图像数量（默认：1） | INT | 是 | 1-6 |
| `种子` | 用于决定节点是否重新运行的种子；实际结果不受种子影响，具有不确定性（默认：0） | INT | 是 | 0-18446744073709551615 |
| `recraft 风格` | 生成背景的可选样式。如未提供，默认使用 "realistic_image" 样式 | STYLEV3 | 否 | - |
| `反向提示词` | 图像中不希望出现的元素的可选文本描述（默认：空） | STRING | 否 | - |

**注意：** `seed` 参数控制节点何时重新执行，但由于外部 API 的特性，无法保证确定性的结果。

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `IMAGE` | 已替换背景的生成图像 | IMAGE |

> 本文档由 AI 生成。如果您发现任何错误或有改进建议，欢迎贡献！ [在 GitHub 上编辑](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/RecraftReplaceBackgroundNode/zh.md)

---
**Source fingerprint (SHA-256):** `305cb8c542159a089b1fa03971205b23d50c8a328af006e284fb27011070f6bd`
