...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `正向` | 用于引导视频生成的正面条件化输入 | CONDITIONING | 是 | - |
| `vae` | 用于将图像编码到潜在空间的 VAE 模型 | VAE | 是 | - |
| `宽度` | 输出视频的宽度（像素）（默认值：848，步长：16） | INT | 是 | 16 至 MAX_RESOLUTION |
| `高度` | 输出视频的高度（像素）（默认值：480，步长：16） | INT | 是 | 16 至 MAX_RESOLUTION |
| `长度` | 输出视频的帧数（默认值：53，步长：4） | INT | 是 | 1 至 MAX_RESOLUTION |
| `批量大小` | 同时生成的视频数量（默认值：1） | INT | 是 | 1 至 4096 |
| `指导类型` | 将起始图像融入视频生成的方法（默认值："v1 (concat)"） | COMBO | 是 | "v1 (concat)"<br>"v2 (replace)"<br>"custom" |
| `起始图像` | 用于初始化视频生成的可选起始图像 | IMAGE | 否 | - |
