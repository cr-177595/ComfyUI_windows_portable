...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `width` | 最终生成图像的宽度。潜在表示的宽度将为此值除以 16。默认值为 1024。 | INT | 是 | 16 至 8192 |
| `height` | 最终生成图像的高度。潜在表示的高度将为此值除以 16。默认值为 1024。 | INT | 是 | 16 至 8192 |
| `batch_size` | 单个批次中生成的潜在样本数量。默认值为 1。 | INT | 否 | 1 至 4096 |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `samples` | 一个填充为零的潜在张量。其形状为 `[batch_size, 128, height // 16, width // 16]`。 | LATENT |
