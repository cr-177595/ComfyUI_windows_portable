...
| `width` | Width of the output video in pixels (default: 832, must be divisible by 16) | INT | Yes | 16 to MAX_RESOLUTION |
| `height` | Height of the output video in pixels (default: 480, must be divisible by 16) | INT | Yes | 16 to MAX_RESOLUTION |
| `length` | Number of frames in the generated video (default: 77, must be divisible by 4) | INT | Yes | 1 to MAX_RESOLUTION |
...
| `latent` | Generated video representation in latent space that can be decoded into final video frames. The latent tensor has shape [batch_size, 16, latent_t, height/8, width/8] where latent_t is derived from the length parameter | LATENT |
