...
| --- | --- | --- | --- | --- |
| `video` | 待超分辨率的输入视频文件。必须为 MP4 容器格式，时长在 5 秒至 10 分钟之间。 | VIDEO | 是 | 不适用 |
| `target_resolution` | 超分辨率输出视频的目标分辨率。 | STRING | 是 | `"720p"`<br>`"1080p"`<br>`"2K"`<br>`"4K"` |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `output` | 以所选目标分辨率输出的超分辨率视频文件。 | VIDEO |
