...
| `宽度` | 生成视频帧的输出宽度（默认值：576，必须能被 8 整除） | INT | 否 | 16 至 MAX_RESOLUTION |
| `高度` | 生成视频帧的输出高度（默认值：576，必须能被 8 整除） | INT | 否 | 16 至 MAX_RESOLUTION |
| `帧数` | 视频序列要生成的帧数（默认值：21） | INT | 否 | 1 至 4096 |
| `俯仰角` | 3D 视图的相机仰角（单位：度，默认值：0.0） | FLOAT | 否 | -90.0 至 90.0 |
