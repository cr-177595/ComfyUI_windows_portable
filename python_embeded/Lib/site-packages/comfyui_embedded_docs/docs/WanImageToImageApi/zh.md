...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `模型` | 要使用的模型（默认值："wan2.5-i2i-preview"）。 | COMBO | 是 | "wan2.5-i2i-preview" |
| `图像` | 单张图像编辑或多张图像融合，最多 2 张图像。 | IMAGE | 是 | - |
| `提示词` | 描述元素和视觉特征的提示词。支持英文和中文（默认值：空）。 | STRING | 是 | - |
| `负面提示词` | 描述要避免内容的负面提示词（默认值：空）。 | STRING | 否 | - |
