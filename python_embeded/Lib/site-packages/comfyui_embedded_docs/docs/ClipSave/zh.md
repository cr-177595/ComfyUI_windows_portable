...
| --- | --- | --- | --- | --- | --- |
| `clip` | 待保存的 CLIP 模型。 | CLIP | 必需 | - | - |
| `文件名前缀` | 保存文件的前缀路径和文件名。节点将附加组件后缀（例如 `_clip_l`、`_clip_g`）和计数器以生成唯一文件名。 | STRING | 必需 | `clip/ComfyUI` | - |
| `prompt` | 工作流提示信息，作为元数据保存在输出文件中。 | PROMPT | 隐藏 | - | - |
| `extra_pnginfo` | 附加元数据，以键值对形式保存在输出文件中。 | EXTRA_PNGINFO | 隐藏 | - | - |
