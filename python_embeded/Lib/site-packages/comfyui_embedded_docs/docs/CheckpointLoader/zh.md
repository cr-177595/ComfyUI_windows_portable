...
| --- | --- | --- | --- | --- |
| `配置名称` | 定义模型架构和设置的配置文件 | STRING | 是 | 可用的配置文件 |
| `Checkpoint名称` | 包含训练后模型权重和参数的检查点文件 | STRING | 是 | 可用的检查点文件 |
...
| 输出名称 | 说明 | 数据类型 |
| --- | --- | --- |
| `MODEL` | 加载后的主模型组件，可用于推理 | MODEL |
| `CLIP` | 加载后的 CLIP 模型组件，用于文本编码 | CLIP |
| `VAE` | 加载后的 VAE 模型组件，用于图像编码和解码 | VAE |
