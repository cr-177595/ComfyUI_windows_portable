...
| --- | --- | --- | --- | --- |
| `clip` | 用于文本分词和编码的 CLIP 模型实例，是生成条件的关键核心。 | CLIP | 是 | - |
| `bert` | 通过 BERT 分词器编码的文本输入。建议使用短语和关键词。支持多行文本和动态提示。 | STRING | 是 | - |
| `mt5xl` | 通过 mT5-XL 分词器编码的文本输入。支持多行文本和动态提示（多语言）。可使用完整句子和复杂描述。 | STRING | 是 | - |

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `CONDITIONING` | 编码后的条件输出，融合了 BERT 和 mT5-XL 两种分词器的文本结果，用于生成任务的后续处理。 | CONDITIONING |
