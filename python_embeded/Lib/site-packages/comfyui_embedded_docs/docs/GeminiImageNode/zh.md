...
| --- | --- | --- | --- | --- | --- |
| `提示词` | 用于生成的文本提示 | STRING | 必需 | "" | - |
| `模型` | 用于生成响应的 Gemini 模型 | COMBO | 必需 | gemini_2_5_flash_image_preview | 可用的 Gemini 模型<br>选项从 GeminiImageModel 枚举中提取 |
| `种子` | 当种子固定为特定值时，模型会尽力为重复请求提供相同的响应。但无法保证确定性输出。此外，更改模型或参数设置（如温度）可能会导致响应发生变化，即使使用相同的种子值也是如此。默认情况下，使用随机种子值 | INT | 必需 | 42 | 0 到 18446744073709551615 |
| `图像` | 可选图像，用作模型的上下文。要包含多个图像，可以使用批量图像节点 | IMAGE | 可选 | None | - |
| `文件` | 可选文件，用作模型的上下文。接受来自 Gemini 生成内容输入文件节点的输入 | GEMINI_INPUT_FILES | 可选 | None | - |
