...
| --- | --- | --- | --- | --- |
| `图像` | 待修改的输入图像 | IMAGE | 是 | - |
| `提示词` | 图像生成的提示词（默认值：""，最大长度：1000 个字符） | STRING | 是 | - |
| `数量` | 生成的图像数量（默认值：1） | INT | 是 | 1-6 |
| `强度` | 定义与原始图像的差异程度，取值范围为 [0, 1]，其中 0 表示几乎相同，1 表示差异极大（默认值：0.5） | FLOAT | 是 | 0.0-1.0 |
| `种子` | 用于确定节点是否应重新执行的种子；实际结果无论种子如何都是非确定性的（默认值：0） | INT | 是 | 0-18446744073709551615 |
| `Recraft风格` | 图像生成的可选样式选择。如果未提供，默认为 `realistic_image` | STYLEV3 | 否 | - |
| `反向提示词` | 图像中不需要元素的可选文本描述（默认值：""） | STRING | 否 | - |
