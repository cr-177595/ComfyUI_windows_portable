...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `video` | 個人用アセットとして登録するビデオ。 | VIDEO | はい | - |
| `group_id` | 既存の Seedance アセットグループ ID を再利用して、同じ人物に対する繰り返しの実在確認をスキップします。空のままにすると、ブラウザで実在認証を実行し、新しいグループを作成します。（デフォルト：空文字列） | STRING | いいえ | - |
