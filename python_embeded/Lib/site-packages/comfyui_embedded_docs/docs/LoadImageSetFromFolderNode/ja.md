...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `folder` | 画像を読み込むフォルダを指定します。 | STRING | はい | 複数のオプションが利用可能 |
| `resize_method` | 画像のリサイズ方法を指定します（デフォルト："None"）。 | STRING | いいえ | "None"<br>"Stretch"<br>"Crop"<br>"Pad" |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `IMAGE` | 読み込まれた画像のバッチを単一のテンソルとして返します。 | IMAGE |
