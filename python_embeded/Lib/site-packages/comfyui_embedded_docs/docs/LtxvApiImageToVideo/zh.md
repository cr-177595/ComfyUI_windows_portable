...
| --- | --- | --- | --- | --- |
| `图像` | 用作视频的第一帧图像。 | IMAGE | 是 | - |
| `模型` | 用于视频生成的 AI 模型。"Fast"模型针对速度进行了优化，而"Quality"模型则优先考虑视觉保真度。 | COMBO | 是 | `"LTX-2 (Fast)"`<br>`"LTX-2 (Quality)"` |
| `提示词` | 用于指导生成视频内容和运动的文本描述。 | STRING | 是 | - |
| `时长` | 视频时长（秒），默认值为 8。 | COMBO | 是 | `6`<br>`8`<br>`10`<br>`12`<br>`14`<br>`16`<br>`18`<br>`20` |
| `分辨率` | 生成视频的输出分辨率。 | COMBO | 是 | `"1920x1080"`<br>`"2560x1440"`<br>`"3840x2160"` |
| `帧率` | 视频的帧率（每秒帧数），默认值为 25。 | COMBO | 是 | `25`<br>`50` |
