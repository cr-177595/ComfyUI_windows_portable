...
| --- | --- | --- | --- | --- |
| `宽度` | 潜空间视频的宽度（像素），默认值：848，必须能被 16 整除 | INT | 是 | 16 至 MAX_RESOLUTION |
| `高度` | 潜空间视频的高度（像素），默认值：480，必须能被 16 整除 | INT | 是 | 16 至 MAX_RESOLUTION |
| `长度` | 潜空间视频的帧数，默认值：25，减 1 后必须能被 6 整除 | INT | 是 | 7 至 MAX_RESOLUTION |
| `批量大小` | 每批次生成的潜空间视频数量，默认值：1 | INT | 否 | 1 至 4096 |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `samples` | 一个具有指定维度的空潜空间视频张量，所有值均为零 | LATENT |
