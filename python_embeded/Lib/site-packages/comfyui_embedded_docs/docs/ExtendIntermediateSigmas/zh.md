...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `sigmas` | 需要扩展中间值的输入 sigma 序列 | SIGMAS | 是 | - |
| `步数` | 在现有 sigma 之间插入的中间步数（默认值：2） | INT | 是 | 1 到 100 |
| `起始 sigma` | 扩展的上限 sigma 边界——仅扩展低于此值的 sigma（默认值：-1.0，表示无穷大） | FLOAT | 是 | -1.0 到 20000.0 |
| `结束 sigma` | 扩展的下限 sigma 边界——仅扩展高于此值的 sigma（默认值：12.0） | FLOAT | 是 | 0.0 到 20000.0 |
| `间距方式` | 用于间隔中间 sigma 值的插值方法（默认值："linear"） | COMBO | 是 | `"linear"`<br>`"cosine"`<br>`"sine"` |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `sigmas` | 已插入额外中间值的扩展 sigma 序列 | SIGMAS |
