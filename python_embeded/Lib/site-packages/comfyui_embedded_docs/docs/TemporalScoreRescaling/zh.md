...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `模型` | 需要应用TSR函数进行修补的扩散模型。 | MODEL | 是 | - |
| `TSR_K` | 控制重缩放强度。在图像生成中，较低的k值产生更详细的细节；较高的k值产生更平滑的结果。设置k=1可禁用重缩放。（默认值：0.95） | FLOAT | 否 | 0.01 - 100.0 |
| `tsr_sigma` | 控制重缩放生效的起始时间。数值越大，生效时间越早。（默认值：1.0） | FLOAT | 否 | 0.01 - 100.0 |
