# 创建约束模型为LoRA（仅模型）

此节点创建一个钩子，将 LoRA（低秩适配）模型应用于神经网络，仅修改模型的组件部分。它会加载一个检查点文件，并以指定的强度将其应用于模型，同时保持 CLIP 组件不变。这是一个实验性节点，扩展了基础 CreateHookModelAsLora 类的功能。

## 输入

| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `Checkpoint名称` | 作为 LoRA 模型加载的检查点文件。可用选项取决于检查点文件夹的内容。 | STRING | 是 | 提供多个选项 |
| `模型强度` | 将 LoRA 应用于模型组件的强度乘数（默认值：1.0） | FLOAT | 是 | -20.0 到 20.0 |
| `前一个约束` | 可选的先前钩子，用于与此钩子链接 | HOOKS | 否 | - |

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `hooks` | 创建的钩子组，包含 LoRA 模型修改 | HOOKS |

> 本文档由 AI 生成。如果您发现任何错误或有改进建议，欢迎贡献！ [在 GitHub 上编辑](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/CreateHookModelAsLoraModelOnly/zh.md)

---
**Source fingerprint (SHA-256):** `adbeaede65aa89d48c59225ca1c8edc4c9394a364f93a00dae4a83a2270f093b`
