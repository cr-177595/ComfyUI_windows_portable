...
| `图像权重` | 图像权重；数值越接近 1.0，图像修改程度越小（默认值：0.1）。内部会将此值反转（1.0 - image_weight）并限制在 0.0 到 0.98 之间。 | FLOAT | 否 | 0.0-0.98 |
| `模型` | 用于图像修改的 Luma 模型。不同模型的成本不同。 | STRING | 是 | `"photon-flash-1"`<br>`"photon-1"`<br>`"photon"` |
| `随机种` | 用于确定节点是否应重新运行的种子值；实际结果与种子无关，具有不确定性（默认值：0） | INT | 否 | 0-18446744073709551615 |

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `图像` | 由 Luma 模型生成的修改后图像 | IMAGE |
