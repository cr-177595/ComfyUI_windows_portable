...
> - 本文檔的內容與 Load3D 節點相同，因為除了動畫預覽和播放之外，它們的功能完全相同。

**支援的格式**
目前，此節點支援多種 3D 檔案格式，包括 `.gltf`、`.glb`、`.obj`、`.fbx` 和 `.stl`。

**3D 節點偏好設定**
可以在 ComfyUI 的設定選單中配置 3D 節點的一些相關偏好設定。請參考以下文檔進行相應設定：
...
![Load 3D 節點 UI](../Load3D/asset/load3d_ui.jpg)
...
在畫布中，一些設定隱藏在選單中。點擊選單按鈕可以展開不同的選單
...
![選單_模型選單_相機](https://raw.githubusercontent.com/Comfy-Org/embedded-docs/refs/heads/main/comfyui_embedded_docs/docs/Load3d/asset/menu_camera.webp)

此選單提供正交視圖和透視視圖之間的切換，以及透視角度大小設定

1. **相機**：在正交視圖和正交視圖之間快速切換
2. **視野（FOV）**：調整視野角度

#### 燈光

![選單_模型選單_相機](https://raw.githubusercontent.com/Comfy-Org/embedded-docs/refs/heads/main/comfyui_embedded_docs/docs/Load3d/asset/menu_light.webp)
