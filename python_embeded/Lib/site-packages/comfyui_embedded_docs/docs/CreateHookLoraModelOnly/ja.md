...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `lora_name` | loras フォルダから読み込む LoRA ファイルの名前 | STRING | はい | 複数のオプションから選択可能 |
| `strength_model` | モデルコンポーネントに LoRA を適用する際の強度倍率（デフォルト：1.0） | FLOAT | はい | -20.0 ～ 20.0 |
| `prev_hooks` | このフックと連鎖させるオプションの以前のフック | HOOKS | いいえ | - |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `hooks` | 作成された LoRA フック。モデル処理に適用できます。 | HOOKS |
