...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `长度` | 空音频片段的时长（秒）（默认值：60.0） | FLOAT | 是 | 0.0 至 1.8446744073709552e+19 |
| `采样率` | 空音频片段的采样率（默认值：44100） | INT | 是 | 1 至 192000 |
| `通道` | 音频声道数（1 为单声道，2 为立体声）（默认值：2） | INT | 是 | 1 至 2 |

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `AUDIO` | 生成的静音音频片段，包含波形数据和采样率信息 | AUDIO |
