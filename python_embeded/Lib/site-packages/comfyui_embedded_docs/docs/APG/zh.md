...
| `eta` | 控制平行引导向量的缩放比例。设置为 1 时等同于默认 CFG 行为（默认值：1.0）。 | FLOAT | 是 | -10.0 到 10.0 |
| `向量归一化` | 将引导向量归一化至此值，设置为 0 时禁用归一化（默认值：5.0）。 | FLOAT | 是 | 0.0 到 50.0 |
| `动量` | 控制扩散过程中引导的运行平均值，设置为 0 时禁用（默认值：0.0）。 | FLOAT | 是 | -5.0 到 1.0 |

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `模型` | 返回经过修改的模型，其采样过程已应用自适应投影引导 | MODEL |
