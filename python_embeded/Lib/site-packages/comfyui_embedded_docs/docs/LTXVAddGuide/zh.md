...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `正向` | 待通过关键帧引导修改的正向条件输入 | CONDITIONING | 是 | - |
| `负向` | 待通过关键帧引导修改的负向条件输入 | CONDITIONING | 是 | - |
| `vae` | 用于编码输入图像/视频帧的 VAE 模型 | VAE | 是 | - |
| `潜在空间` | 将接收条件帧的输入潜在序列 | LATENT | 是 | - |
| `图像` | 用于对潜在视频进行条件处理的图像或视频。必须为 8*n + 1 帧。如果视频不是 8*n + 1 帧，将裁剪至最近的 8*n + 1 帧。 | IMAGE | 是 | - |
| `帧索引` | 开始条件处理的帧索引。对于单帧图像或 1-8 帧的视频，任何 `帧索引` 值均可接受。对于 9 帧及以上的视频，`帧索引` 必须能被 8 整除，否则将向下取整至最近的 8 的倍数。负值从视频末尾开始计数。（默认值：0） | INT | 否 | -9999 至 9999 |
| `强度` | 条件影响的强度，1.0 表示完全应用条件，0.0 表示不应用条件（默认值：1.0） | FLOAT | 否 | 0.0 至 1.0 |
