...
| --- | --- | --- | --- | --- |
| `model_name` | 来自 `models/detection/` 目录的人脸检测模型。 | STRING | 是 | `models/detection/` 目录中可用的模型列表 |

## ## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `FACE_DETECTION_MODEL` | 一个已加载的 FaceLandmarker 模型对象，包含两种检测变体（短距离/全距离）、面部拓扑连接集、标准数据以及用于 GPU 管理的模型修补器。 | FACE_DETECTION_MODEL |
