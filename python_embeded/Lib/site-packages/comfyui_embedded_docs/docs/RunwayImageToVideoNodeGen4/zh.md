...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `提示词` | 用于生成的文本提示（默认：空字符串） | STRING | 是 | - |
| `起始帧` | 用于视频的起始帧 | IMAGE | 是 | - |
| `时长` | 视频时长（秒）（默认："5"） | COMBO | 是 | `"5"`<br>`"10"` |
| `比例` | 生成视频的宽高比（默认："1024:1024"） | COMBO | 是 | `"1024:1024"`<br>`"1280:720"`<br>`"720:1280"`<br>`"1920:1080"`<br>`"1080:1920"`<br>`"2048:1080"`<br>`"1080:2048"` |
| `种子` | 用于生成的随机种子（默认：0） | INT | 否 | 0 到 4294967295 |
