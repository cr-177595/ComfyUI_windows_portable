...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `meshy_task_id` | 先前 Meshy 操作（例如，文本转 3D 或图像转 3D）生成的、待绑定模型的唯一任务 ID。 | STRING | 是 | 无 |
| `height_meters` | 角色模型的近似高度（以米为单位）。这有助于提高缩放和绑定的准确性（默认值：1.7）。 | FLOAT | 是 | 0.1 至 15.0 |
| `texture_image` | 模型的 UV 展开基础颜色纹理图像。 | IMAGE | 否 | 无 |
