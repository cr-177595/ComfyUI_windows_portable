...
| --- | --- | --- | --- | --- |
| `model_3d` | 待处理的3D模型。模型应为FBX格式，且面数少于30000。 | FILE3D | 是 | FBX, 任意 |
| `seed` | 用于控制节点是否重新运行的种子值。无论种子值如何，结果均具有非确定性。（默认值：0） | INT | 否 | 0 至 2147483647 |
...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `FBX` | 处理后的3D模型，以FBX文件形式返回。 | FILE3DFBX |
