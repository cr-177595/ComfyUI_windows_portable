...
| --- | --- | --- | --- | --- |
| `系数` | 控制噪声调度曲线的系数值（默认值：1.20） | FLOAT | 是 | 0.80 - 1.50 |
| `步数` | 生成 sigma 值的总采样步数（默认值：10） | INT | 是 | 2 - 1000 |
| `降噪` | 减少实际使用步数的去噪因子（默认值：1.0） | FLOAT | 是 | 0.0 - 1.0 |
