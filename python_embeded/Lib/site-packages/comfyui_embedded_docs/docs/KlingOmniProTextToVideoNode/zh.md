...
| `提示词` | 描述视频内容的文本提示词。可包含正面和负面描述。启用故事板时忽略此参数。 | STRING | 是 | 0 到 2500 个字符 |
| `宽高比` | 生成视频的画幅或尺寸。 | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` |
| `时长` | 视频的时长（以秒为单位，默认值：5）。 | INT | 是 | 3 到 15 秒 |
| `分辨率` | 视频的质量或像素分辨率（默认值：`"1080p"`）。 | COMBO | 否 | `"4k"`<br>`"1080p"`<br>`"720p"` |
| `分镜` | 生成一系列带有独立提示词和时长的视频片段。o1 模型忽略此参数。 | DYNAMIC_COMBO | 否 | `"disabled"`<br>`"1 storyboard"`<br>`"2 storyboards"`<br>`"3 storyboards"`<br>`"4 storyboards"`<br>`"5 storyboards"`<br>`"6 storyboards"` |
