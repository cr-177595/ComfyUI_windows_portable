...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `正面条件` | 引导生成朝向期望内容的正向条件 | CONDITIONING | 是 | - |
| `负面条件` | 引导生成远离不期望内容的负向条件 | CONDITIONING | 是 | - |
| `ControlNet` | 提供额外生成控制的 ControlNet 模型 | CONTROL_NET | 是 | - |
| `vae` | 用于图像编码和解码的 VAE（变分自编码器） | VAE | 是 | - |
| `图像` | 作为 ControlNet 控制引导的输入图像 | IMAGE | 是 | - |
| `遮罩` | 定义图像中需要修复区域的遮罩 | MASK | 是 | - |
| `强度` | ControlNet 效果的强度（默认值：1.0） | FLOAT | 是 | 0.0 至 10.0 |
| `开始百分比` | ControlNet 影响在生成过程中开始的起点（以百分比表示，默认值：0.0） | FLOAT | 是 | 0.0 至 1.0 |
| `结束百分比` | ControlNet 影响在生成过程中结束的终点（以百分比表示，默认值：1.0） | FLOAT | 是 | 0.0 至 1.0 |
